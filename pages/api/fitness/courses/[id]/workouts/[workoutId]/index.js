...
export default async function handler(req, res) {
    ...
    // id — это courseId (из родительской папки [id])
    // workoutId — из папки [workoutId]
    const { id: courseId, workoutId } = req.query;
    ...
    if (!req.body)
        return res.status(400).json({
            message:
                "Тело запроса должно быть не пустым и содержать progressData",
        });
    const { progressData } = JSON.parse(req.body);

    try {
        await markWorkoutProgress(
            decoded.id,
            courseId,
            workoutId,
            progressData,
